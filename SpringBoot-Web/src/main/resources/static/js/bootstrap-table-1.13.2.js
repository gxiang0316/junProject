(function (global, factory) {
  ...
  (function ($) {
    ...
    var bootstrap = {
      3: {
        ...
        html: {
          ...
          pageDropdown: ['<ul class="dropdown-menu" style="width:100px;" role="menu">', '</ul>'],
    ...
    var DEFAULTS = {
      ...
      /**自定义属性*/
      paginationCRUD:false,  // 自定义属性 判断是否显示分页栏增删改查按钮 控制下面几个按钮
      paginationAddClass:'',// 如果不想显示 设置为pagination-crud-hide 单个控制
      paginationUpdate:'',// 如果不想显示 设置为pagination-crud-hide
      paginationDelete:'',// 如果不想显示 设置为pagination-crud-hide
      paginationSpare:'',// 备用按钮 如果不想显示 设置为pagination-crud-hide
      paginationExport:'',// 如果不想显示 设置为pagination-crud-hide
      /**自定义属性*/
      pagination: false,
      ...
      buttonsAlign: 'right',
      buttonsClass: 'custom-dropdown-blue',// 修改每页显示数下拉按钮默认样式
      ...
      onClickRow: function onClickRow(item, $element) {
        if(DEFAULTS.paginationCRUD){
            // 判断是否分页栏有增删改查等操作，如果有 修改点击行时该行背景色
      ...
      onDelete: function onDelete(rows) {// rows 参数对客户端分页有效
        return false;
      },
    ...
    var EVENTS = {
      ...
      'onClickAdd.bs.table':'onAdd',
      'onClickUpdate.bs.table':'onUpdate',
      'onClickDelete.bs.table':'onDelete',
      'onClickSpare.bs.table':'onSpare',
      'onClickExport.bs.table':'onExport'
    };
    ...
    var BootstrapTable = function () {
      ...
      _createClass(BootstrapTable, [{
        ...
        value: function initPagination() {
          ...
          var $c_next = void 0;
          var $c_last = void 0;
          var $c_pageInput = void 0;
          var $c_refresh = void 0;
          var $c_add = void 0;
          var $c_update = void 0;
          var $c_delete = void 0;
          var $c_spare = void 0;
          ...
          html.push(Utils.sprintf('<div class="%s-%s pagination-detail">',bootstrap.classes.pull, this.options.paginationDetailHAlign),
          ...
          if (!this.options.onlyInfoPagination) {
           /**开始生成 分页栏按钮*/
            if(this.options.paginationStyle == 'jqGrid'){
                // row fixed-table-pagination
                // pagination align-items-center justify-content-center order-2 col-md-12 col-lg-4
                html.push('</div>', Utils.sprintf('<div class="%s-%s pagination align-items-center justify-content-center">',
                                              bootstrap.classes.pull, this.options.paginationHAlign));
                html.push('<a id="c_first" class="glyphicon glyphicon-step-backward pr-0"></a>');
                html.push('<a id="c_pre" class="glyphicon glyphicon-backward pl-1 pr-1"></a>');
                html.push('<span class="ui-separator ml-2 mr-1"></span>');

                html.push('<div class="flex-column" style="margin-right: 3px;margin-left: 3px">');
                html.push('第 ');
                html.push('<input id="currentPageNumInput" class="ui-pg-input form-control" ' +
                    'style="padding-left: 5px;width: 50px;" type="text" value="'+this.options.pageNumber+'">');
                html.push(' 页');
                html.push('&nbsp;&nbsp;&nbsp;总');
                html.push('<span id="sTotalPage">'+this.options.totalPages+'</span>');
                html.push('页');
                html.push('</div>');

                html.push('<span class="ui-separator ml-1 mr-2"></span>');
                html.push('<a id="c_next" class="glyphicon glyphicon-forward pl-1 pr-1"></a>');
                html.push('<a id="c_last" class="glyphicon glyphicon-step-forward pl-0"></a>');
                ...
                if (from > 1) {
                    ...
                    for (i = 1; i <= max; i++) {html.push(pageItem(i));}
                    if (from - 1 === max + 1) {
                        i = from - 1;
                ...
                if (this.totalPages > to) {
                    ...
                    if (to + 1 === min - 1) {
                        ...
                        if (min > to + 1) {
                            if (this.totalPages - to > this.options.paginationPagesBySide * 2
          ...
          if (typeof this.options.pageList === 'string') {
              ...
              try {
                  for (var _iterator9 = list[Symbol.iterator](), _step9;
          ...
          // 就这句生成 ‘ 每页显示 下拉框按钮  条记录 ’
          html.push(this.options.formatRecordsPerPage(pageNumber.join('')));
          html.push('</span>');
          html.push('<span class="ui-separator ml-2 mr-1"></span>');
          // 此刷新跟默认的刷新一样，调用默认刷新的方法
          html.push('<a id="c_refresh" class="glyphicon glyphicon-refresh mr-1"></a>');
          if(this.options.paginationCRUD){
              /**是否显示 增删改查 导出*/
              html.push('<span class="ui-separator ml-0 mr-2"></span>');
              html.push(Utils.sprintf('<a id="c_add" class="glyphicon glyphicon-plus pl-0 %s"></a>',this.options.paginationAddClass));
              html.push(Utils.sprintf('<a id="c_update" class="glyphicon glyphicon-pencil pl-0 %s"></a>',this.options.paginationUpdate));
              html.push(Utils.sprintf('<a id="c_delete" class="glyphicon glyphicon-trash pl-0 %s"></a>',this.options.paginationDelete));
              html.push(Utils.sprintf('<a id="c_spare" class="glyphicon glyphicon-asterisk pl-0 %s"></a>',this.options.paginationSpare));
              html.push(Utils.sprintf('<a id="c_export" class="glyphicon glyphicon-floppy-save pl-0 %s"></a>',this.options.paginationExport));
          }
          ...
          if (!this.options.onlyInfoPagination) {
              if(this.options.paginationStyle == 'jqGrid'){
                  $c_first = this.$pagination.find("#c_first");
                  $c_pre = this.$pagination.find("#c_pre");
                  $c_next = this.$pagination.find("#c_next");
                  $c_last = this.$pagination.find("#c_last");
                  $c_pageInput = this.$pagination.find("#currentPageNumInput");

                  $c_first.off('click').on('click', $.proxy(this.onCPageNumber,this));
                  $c_pre.off('click').on('click', $.proxy(this.onCPagePre, this));
                  $c_next.off('click').on('click', $.proxy(this.onCPageNext, this));
                  $c_last.off('click').on('click', $.proxy(this.onCPageNumber,this));
                  $c_pageInput.off('keypress').on('keypress', function (event) {
                      if(event.keyCode == 13){
                          _this6.onCPageNumber(event);
                          // $.proxy(_this6.onCPageNumber,event);
                          //alert('你输入的内容为1：' + $(event.currentTarget).val());
                      }
                  });
              }else{
              ...
              // 刷新按钮
              $c_refresh = this.$pagination.find('#c_refresh');
              $c_refresh.off('click').on('click', $.proxy(this.refresh, this));
              // 选择每页显示多少条 下拉框选择事件
              $pageList = this.$pagination.find('.page-list a');
              $pageList.off('click').on('click', $.proxy(this.onPageListChange, this));

              if(this.options.paginationCRUD){
                  $c_add = this.$pagination.find("#c_add");
                  $c_update = this.$pagination.find("#c_update");
                  $c_delete = this.$pagination.find("#c_delete");
                  $c_spare = this.$pagination.find("#c_spare");
                  $c_export = this.$pagination.find("#c_export");
                  $c_add.off('click').on('click', function () {
                      _this6.trigger('onClickAdd');
                  });
                  $c_update.off('click').on('click', function () {
                      _this6.paginationUpdate();
                  });
                  $c_delete.off('click').on('click', function () {
                      _this6.paginationDelete();
                  });
                  $c_spare.off('click').on('click', function () {
                      _this6.trigger('onClickSpare');
                  });
                  $c_export.off('click').on('click', function () {
                      _this6.trigger('onClickExport');
                  });
              }
            ...
            if (!this.options.paginationLoop) {
              if (this.options.pageNumber === 1) {
                  if(this.options.paginationStyle == 'jqGrid'){
                      $c_first.addClass('disabled');
                      $c_pre.addClass('disabled');
                      $c_first.css('color','rgba(128, 128, 128, 0.69)');
                      $c_pre.css('color','rgba(128, 128, 128, 0.69)');
                  }else{
              ...
              if (this.options.pageNumber === this.totalPages) {
                  if(this.options.paginationStyle == 'jqGrid'){
                      ...
                      $c_last.addClass('disabled');
                      $c_next.css('color','rgba(128, 128, 128, 0.69)');
                      $c_last.css('color','rgba(128, 128, 128, 0.69)');
                  }else{
        ...
        key: 'onCPagePre',
        value: function onCPagePre(event) {
          event.preventDefault();
          if (this.options.pageNumber - 1 === 0) {
            //this.options.pageNumber = 1;
              return;
        ...
        key: 'onCPageNext',
        value: function onCPageNext(event) {
          event.preventDefault();
          if (this.options.pageNumber + 1 > this.options.totalPages) {
              return;
            //this.options.pageNumber = this.options.totalPages;
          ...
          key: 'onCPageNumber',
          value: function onCPageNumber(event) {
              event.preventDefault();
              if($(event.currentTarget).hasClass('glyphicon-step-backward')){// 第一页
                  if (this.options.pageNumber == 1) {
                      return;
                  }
                  this.options.pageNumber = 1;
              }else if($(event.currentTarget).hasClass('glyphicon-step-forward')){// 最后一页
        ...
        value: function updateFormatText(name, text) {
          ...
          value: function paginationUpdate() {
              ...
              console.log("更新时选择了多少项 ："+rows.length);
              if(rows && jQuery.isArray(rows) && rows.length > 1){
                  // 是数组 且当前选中了不止一个
                  this.trigger('onClickUpdate', rows, false);
              }else if(rows && jQuery.isArray(rows) && rows.length == 1){
                  this.trigger('onClickUpdate', rows[0], true);
              }

          }
      },{
          key: 'paginationDelete',
          value: function paginationDelete() {
              var rows = this.getSelections();
              console.log("删除时选择了多少项 ："+rows.length);
              this.trigger('onClickDelete', rows);
          }
      }]);
